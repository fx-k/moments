<template>
  <UModal
    ...
    :ui="{
      container:
        'fixed inset-0 flex justify-center items-center bg-black/35 dark:bg-black/55 backdrop-blur-sm p-4',
    }"
  >
    <div class="moments-modal-panel w-[min(92vw,520px)]">
      <button class="moments-modal-close" type="button" title="关闭" @click="open = false">
        <UIcon name="i-carbon-close" class="w-4 h-4" />
      </button>
      <div class="p-5">
        <div
          v-if="global.userinfo.token"
          class="flex flex-col items-center pt-2 text-gray-500 dark:text-white"
          @click="navigate('/new')"
          title="发表"
        >
        ...
        <div class="flex items-center justify-center gap-3 pt-4 text-gray-500 dark:text-white min-h-[120px]">
